...
import { emptyGroup, GroupColumnConfig, groupHeaders, practiceHeaders, RoleColumnConfig, roleHeaders } from '@/app/Constants/Constants';
...
export default function GroupAdminDashboard() {
  ...
  const _practiceService = new GroupService();
  const [allItems, setAllItems] = useState<IUserGroup[]>([]);
  const [sortedData, setSortedData] = useState<IUserGroup[]>([]);
  ...
  const [selectedGroup, setSelectedGroup] = useState<IUserGroup>(emptyGroup);
  ...
  const getData = async () => {
    ...
    try {
      ...
      const result = await _practiceService.getGroups();
  ...
  const getSortedData = async (data: any) => {
    return await sortTableData(data, "userName", "asc") as IUserGroup[];
  }
  ...
  const setLoaderAndSortedData = (loader: boolean, sortData: IUserGroup[]) => {
    setIsLoading(loader);
...
const setCurrentItem=(create:boolean)=>{
  ...
  const deleteSelected = async () => {
    ...
    try {
      ...
      const result = await _practiceService.deleteGroups(selectedItemsArray);
  ...
  return (
    <div className='px-3 flex flex-1 flex-column overflow-hidden la-table-styles h-100 role-select-none role-access-config'>
      ...
      <TableBody<IUserGroup> sortOrder={sortOrder} setSortOrder={setSortOrder} sortTableData={sortTableData} selectedItems={selectedItems} getData={getData} sortedData={sortedData} isLoading={isLoading} setLoaderAndSortedData={setLoaderAndSortedData} handleRowCheckboxChange={handleCheckboxChange} defaultSortedColumn={defaultSortedColumn} sortedColumn={sortedColumn} setSortedColumn={setSortedColumn} itemsPerPage={itemsPerPage} setItemsPerPage={setItemsPerPage} currentPage={currentPage} setCurrentPage={setCurrentPage} tableHeaders={groupHeaders} columnConfig={GroupColumnConfig} />
      <GroupForm selectedGroup={selectedGroup} isEdit={isEdit} isCreate={isCreate} clearForm={clearForm} save={submitForm} />
      {true && (
        <ConfirmPopup
