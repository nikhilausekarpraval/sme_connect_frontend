...
import { emptyPractice, PracticeColumnConfig, practiceHeaders, RoleColumnConfig, roleHeaders } from '@/app/Constants/Constants';
...
import PracticeForm from '../Forms/PracticeForm/PracticeForm';

export default function PracticeAdminDashboard() {

  const _practiceService = new PracticesService();
  const [allItems, setAllItems] = useState<IPractice[]>([]);
  const [sortedData, setSortedData] = useState<IPractice[]>([]);
  ...
  const defaultSortedColumn = "name";
  ...
  const [selectedPractice, setSelectedPractice] = useState<IPractice>(emptyPractice);
  ...
  const getData = async () => {
    ...
    try {
      ...
      const result = await _practiceService.getPractices();
  ...
  const getSortedData = async (data: any) => {
    return await sortTableData(data, "userName", "asc") as IPractice[];
  }
  ...
  const setLoaderAndSortedData = (loader: boolean, sortData: IPractice[]) => {
    setIsLoading(loader);
...
const setCurrentItem=(create:boolean)=>{
    try {
      if (create) {
        setSelectedPractice(emptyPractice);
        return;
      }
      const id = Array.from(selectedItems)[0];
      const emp = allItems.filter(item => item.id === id)[0];
      setSelectedPractice(emp);
    } catch (e: any) {
  ...
  const deleteSelected = async () => {
    ...
    try {
      ...
      const result = await _practiceService.deletePractices(selectedItemsArray);
  ...
  return (
    <div className='px-3 flex flex-1 flex-column overflow-hidden la-table-styles h-100 role-select-none role-access-config'>
      ...
      <TableBody<IPractice> sortOrder={sortOrder} setSortOrder={setSortOrder} sortTableData={sortTableData} selectedItems={selectedItems} getData={getData} sortedData={sortedData} isLoading={isLoading} setLoaderAndSortedData={setLoaderAndSortedData} handleRowCheckboxChange={handleCheckboxChange} defaultSortedColumn={defaultSortedColumn} sortedColumn={sortedColumn} setSortedColumn={setSortedColumn} itemsPerPage={itemsPerPage} setItemsPerPage={setItemsPerPage} currentPage={currentPage} setCurrentPage={setCurrentPage} tableHeaders={practiceHeaders} columnConfig={PracticeColumnConfig} />
      <PracticeForm selectedPractice={selectedPractice} isEdit={isEdit} isCreate={isCreate} clearForm={clearForm} save={submitForm} />
      {true && (
        <ConfirmPopup
