 ...
 const isSortedColumn = () => {
    ...
    return (
      <th scope="col" className="border-bottom-0 position-sticky top-0" onClick={()=>handleSort(value)}>
        <div className={`d-flex ${name.includes("On Dt") ? 'audit-column-width' : name === "action" ? "justify-center" : ""} ${getDataTypeForKey(value,conlumnConfig) === 'number' ? 'justify-content-end' : '' }`}>
          {notSortable ? (
            <span>{name}</span>
          ) : (
              <React.Fragment>
